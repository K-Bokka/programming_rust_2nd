...
impl<T> GapBuffer<T> {
    ...
    unsafe fn space(&self, i: usize) -> *const T {
        self.storage.as_ptr().offset(i as isize)
    }

    #[allow(unsafe_op_in_unsafe_fn)]
    unsafe fn space_mut(&mut self, i: usize) -> *mut T {
        self.storage.as_mut_ptr().offset(i as isize)
    }

    #[allow(dead_code)]
    fn index_to_raw(&self, i: usize) -> usize {
        if i < self.gap.start {
            i
        } else {
            i + self.gap.len()
    ...
    pub fn insert_iter<I>(&mut self, iter: I)
    where
